...
from charm import CloudflareAcmeOperatorCharm


class TestCharm(unittest.TestCase):
    def setUp(self):
        self.harness = Harness(CloudflareAcmeOperatorCharm)
        self.addCleanup(self.harness.cleanup)
        self.harness.begin()

    def test_given_email_is_valid_when_config_changed_then_status_is_active(self):
        self.harness.update_config(
            {"email": "example@email.com", "cloudflare_api_key": "dummy key"}
        )
        self.assertEqual(self.harness.model.unit.status, ActiveStatus())

    def test_given_email_is_invalid_when_config_changed_then_status_is_blocked(self):
        self.harness.update_config({"email": "invalid-email", "cloudflare_api_key": "dummy key"})
